...
try:
    ...
    print("Audio files not found. Sounds will be disabled.")
    wing_sound = None
    hit_sound = None
...
class FlappyBirdEnv:
    ...
    def step(self, action, render_game=True):
        ...
        if (pygame.sprite.groupcollide(self.bird_group, self.ground_group, False, False, pygame.sprite.collide_mask) or
                ...
                self.bird.rect[1] < 0): # Check if bird goes above the screen
            if hit_sound:
                hit_sound.play()
...
class DQNAgent:
    def __init__(self, state_size, action_size, seed=0, model_path='models/flappy_bird_dqn.pth'):
        ...
        self.memory = ReplayBuffer(capacity=100000) # Increased replay buffer capacity
        self.gamma = 0.99
        self.epsilon = 1.0 # Initial epsilon value, will be overwritten if model loaded
    ...
    def load_model(self):
        if os.path.exists(self.model_path):
            ...
            if isinstance(checkpoint, dict) and 'qnetwork_state_dict' in checkpoint:
                ...
                self.epsilon = 0.5 # Starting with 0.5 for exploration if old model format
...
def main():
    ...
    num_episodes = 5000 # Increased number of training episodes
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        if i_episode % 50 == 0: # Print progress every 50 episodes
            print(f"Episode {i_episode}/{num_episodes}, Average Score (last 50): {np.mean(scores[-50:]):.2f}, Epsilon: {agent.epsilon:.4f}")
    ...
    for i_episode in range(1, num_demo_episodes + 1):
        ...
        while not done:
            ...
            state = next_state
            score += reward
        print(f"Demo Episode {i_episode}/{num_demo_episodes}, Score: {score:.2f}")
